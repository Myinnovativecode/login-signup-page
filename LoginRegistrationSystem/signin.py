...
def forget_pass():
    def change_password():
        if user_entry.get()=='' or newpass_entry.get()=='' or confirmpass_entry.get()=='':
            ...
            con = pymysql.connect(host='localhost', user='root', password='Mysql2024#',database='userdata')
            mycursor = con.cursor()
    ...
    userLabel = Label(window, text="USER LOGIN", font=('arial',12, 'bold')
                    , bg='white', fg='orchid1')
    ...
    confirmpass_entry = Entry(window, width=25, font=('arial', 11, 'bold'), bd=0
                             , fg='magenta2')
    confirmpass_entry.place(x=470, y=320)

    Frame(window, width=250, height=2, bg='orchid1').place(x=470, y=340)

    signupButton = Button(window, text='Submit', font=('Open Sans', '16', 'bold'), bd=0, bg='magenta2', fg='white',
                         cursor='hand2' , activebackground='magenta2', activeforeground='white', width=19,command=change_password)
    signupButton.place(x=470,y=390)
...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
            con=pymysql.connect(host='localhost', user='root', password='Mysql2024#')
            mycursor=con.cursor()
        except:
            messagebox.showerror('Error','Connection is not established again')
            return
        query = 'use userdata'
...
heading=Label(login_window,text="USER LOGIN",font=('Microsoft Yahel UI Light',23,'bold')
              ,bg='white',fg='firebrick1')
heading.place(x=605,y=120)
usernameEntry=Entry(login_window,width=25,font=('Microsoft Yahel UI Light',11,'bold')
                    ,bd=0,fg='firebrick1')
...
passwordEntry=Entry(login_window,width=25,font=('Microsoft Yahel UI Light',11,'bold')
                    ,bd=0,fg='firebrick1')
...
forgetButton=Button(login_window,text='Forgot Password',bd=0,bg='white',activebackground='white'
                 ,cursor='hand2',font=('Microsoft Yahel UI Light',9,'bold'),
